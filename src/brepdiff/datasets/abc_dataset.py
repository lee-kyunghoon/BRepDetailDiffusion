...
def get_face_adjacency(solid: Solid):
    mapper = EntityMapper(solid)
    condition_faces = list(solid.faces())
...
class ABCDataset(BaseDataset):
    ...
    def __init__(self, config: Config, split="train"):
        ...
        if split == "train_for_test":
            split = "train"
            train_for_test = True
            print(f"condition_path: {config.condition_path}")
            solids = load_step(config.condition_path)
            self.face_adj = get_face_adjacency(solids[0])
    ...
    def __getitem__(self, idx) -> ABCDatasetOutput:
        ...
        face_adj = data["face_adj"][:] if "face_adj" in data else None
