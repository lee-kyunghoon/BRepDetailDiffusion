...
class Postprocessor:
    def __init__(
        ...
        self.scaled_uvgrid_extend = self.scaled_uvgrid.extend_tangent(extend_len=self.uvgrid_extend_len, n_extend = 1)
    ...
    def _uvgrid2wn_single_face(self, f_idx: int) -> torch.Tensor:
        ...
        if self.use_fwn:
            wn = igl.fast_winding_number(v, f, meshgrid_np)
        else:
    ...
    def _wn(args: Tuple[np.ndarray, np.ndarray, np.ndarray, bool]) -> np.ndarray:
        ...
        if use_fwn:
            wn = igl.fast_winding_number(v, f, query_coord)
        else:
