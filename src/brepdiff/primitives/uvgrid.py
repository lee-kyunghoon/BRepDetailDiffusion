...
class UvGrid:
    ...
    def __init__(
        ...
        face_adj: Union[np.ndarray, torch.Tensor, None] = None,
    ...
    def from_raw_values(
        ...
        face_adj = UvGrid._pad_face_adj(face_adj, max_n_prims) if face_adj is not None else None
    ...
    def export_npz(self, file_path: str, vis_idx: Union[int, None] = None):
        ...
        if isinstance(self.coord, torch.Tensor):
            ...
            face_adj = self._to_npy(face_adj)
        ...
        if vis_idx is not None:
            ...
            if face_adj is not None:
                save_dict["face_adj"] = face_adj[vis_idx]
        else:
    ...
    def load_from_npz_data(
        ...
        if max_prims is not None:
            uvgrid = UvGrid(
                ...
                face_adj=(
                    npz_data.get("face_adj")[:max_prims, :max_prims]
                    if "face_adj" in npz_data
                    else None
                ),
            )
        else:
            uvgrid = UvGrid(
                ...
                face_adj=(npz_data.get("face_adj") if "face_adj" in npz_data else None),
            )
        return uvgrid
    def upscale_grid(self, new_grid_size: int) -> None:
        """
        coord:     (N, H, W, 3)  — float, 3D 좌표
        grid_mask: (N, H, W)     — bool, surface 유효성
        normal:    (N, H, W, 3)  — float, 법선벡터
        new_res:   목표 해상도 (e.g., 32)
        """

        coord_t = self.coord.permute(0, 3, 1, 2).float()  # (N, 3, H, W)
        coord_up = F.interpolate(
            coord_t, size=(new_grid_size, new_grid_size),
            mode='bilinear', align_corners=True
        )
        self.coord = coord_up.permute(0, 2, 3, 1)  # (N, new_grid_size, new_grid_size, 3)

        mask_t = self.grid_mask.float().unsqueeze(1)   # (N, 1, H, W)
        mask_up = F.interpolate(
            mask_t, size=(new_grid_size, new_grid_size),
            mode='nearest'                         
        )
        self.grid_mask = mask_up.squeeze(1).bool()       # (N, new_grid_size, new_grid_size)

        normal_t = self.normal.permute(0, 3, 1, 2).float()  # (N, 3, H, W)
        normal_up = F.interpolate(
            normal_t, size=(new_grid_size, new_grid_size),
            mode='bilinear', align_corners=True
        )
        normal_up = F.normalize(normal_up, p=2, dim=1) 
        self.normal = normal_up.permute(0, 2, 3, 1)       # (N, new_grid_size, new_grid_size, 3)
    ...
    def extend_tangent(
        ...
        extend_len: float,
        n_extend: int = 1,
        curvature_weight: float = 0.0,  # 0이면 선형과 동일, 높을수록 곡률 강하게 반영
    ) -> UvGrid:
        """
        UV grid를 탄젠트 + 곡률을 반영하여 확장.
        coord: (N, H, W, 3)
        """
        coord = self.coord[~self.empty_mask].clone()  # (N, H, W, 3)

        def get_tangent_and_curvature(p0, p1, p2):
            """
            p0: 경계점        (N, *, 3)
            p1: 경계 1칸 안쪽 (N, *, 3)
            p2: 경계 2칸 안쪽 (N, *, 3)
            
            탄젠트:  p0에서 밖으로 나가는 방향 = p0 - p1
            곡률벡터: 탄젠트의 변화량 = (p0-p1) - (p1-p2)
            """
            t = p0 - p1                        # 1차 탄젠트 벡터
            t_prev = p1 - p2                   # 이전 탄젠트 벡터
            curvature = t - t_prev             # 탄젠트 변화량 (2차 미분)
            return t, curvature

        def extend_one_step(p0, t, curvature, step, extend_len, curvature_weight):
            """한 스텝 확장"""
            # 탄젠트 방향으로 extend_len만큼 이동
            t_norm = F.normalize(t, p=2, dim=-1)
            step_size = torch.norm(t, dim=-1, keepdim=True).mean() * extend_len

            # 곡률 보정: 탄젠트 방향 변화를 누적
            new_pt = p0 + t_norm * step_size + curvature * curvature_weight * (step + 1)
            return new_pt

        # ── U축 (dim=1) 방향 확장 ──────────────────────────────
        # Start 방향 (앞쪽 확장)
        start_pts = []
        p0, p1, p2 = coord[:, 0], coord[:, 1], coord[:, 2]
        t, curv = get_tangent_and_curvature(p0, p1, p2)
        current = p0.clone()
        for step in range(n_extend):
            new_pt = extend_one_step(current, t, curv, step, extend_len, curvature_weight)
            start_pts.insert(0, new_pt.unsqueeze(1))
            current = new_pt

        # End 방향 (뒤쪽 확장)
        end_pts = []
        p0, p1, p2 = coord[:, -1], coord[:, -2], coord[:, -3]
        t, curv = get_tangent_and_curvature(p0, p1, p2)
        current = p0.clone()
        for step in range(n_extend):
            new_pt = extend_one_step(current, t, curv, step, extend_len, curvature_weight)
            end_pts.append(new_pt.unsqueeze(1))
            current = new_pt

        coord = torch.cat(start_pts + [coord] + end_pts, dim=1)
        ...
        for step in range(n_extend):
            new_pt = extend_one_step(current, t, curv, step, extend_len, curvature_weight)
            start_pts.insert(0, new_pt.unsqueeze(2))
            current = new_pt
        ...
        for step in range(n_extend):
            new_pt = extend_one_step(current, t, curv, step, extend_len, curvature_weight)
            end_pts.append(new_pt.unsqueeze(2))
            current = new_pt

        coord = torch.cat(start_pts + [coord] + end_pts, dim=2)

        empty_mask = torch.ones(coord.shape[0], device=coord.device, dtype=torch.bool)
        return UvGrid(coord=coord, empty_mask=empty_mask, grid_mask=None)
    ...
    def copy(self) -> UvGrid:
        ...
        return UvGrid(
            ...
            face_adj=self.face_adj.clone() if self.face_adj is not None else None,
        )
