...
class TwoDit1D(nn.Module):
    ...
    def __init__(
        ...
        data_dim = 3  # coordinate dimension
        high_t_input_dim = int(input_dim * data_dim / per_points_dim)  # coord only
        ...
        # Precompute where coordinates lie in x
    ...
    def forward(
        ...
        # low noise network
        if t_low_mask.sum() != 0:
            z_t_low = z[t_low_mask] if z is not None else None
            mask_t_low = mask[t_low_mask] if mask is not None else None
        ...
        if (~t_low_mask).sum() != 0:
            ...
            z_t_high = z[~t_low_mask] if z is not None else None
            mask_t_high = mask[~t_low_mask] if mask is not None else None
