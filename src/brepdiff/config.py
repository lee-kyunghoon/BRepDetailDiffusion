...
class DiffusionConfig:
    ...
    type_noise_schedule: str = "snr_based"
    type_timesteps: int = 500
...
class Config(YAMLWizard, key_transform="SNAKE"):
    ...
    data_dim: int = 3
    n_grid: int = 8
    condition_path: str = None  # path to the conditioning information, such as class labels or text embeddings. Should be in format of .npy file containing a dict with keys being the uid and values being the conditioning vector.
    max_n_prims: int = (
    ...
    n_face_types: int = 0  # number of discrete face types (0=disabled)
    graph_conditioning: bool = True  # use face-adjacency graph encoder for conditioning z
    graph_encoder_type: str = "gcn"  # ["gcn", "gat"]
    graph_num_layers: int = 2
    graph_hidden_dim: int = 256
    graph_pool: str = "mean"  # ["mean", "max"]

    # Custom STEP dataset settings
    custom_dataset_name: str = "custom"
    custom_max_n_prims: int = 50
